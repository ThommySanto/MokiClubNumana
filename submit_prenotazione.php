...
$stmt = $conn->prepare("SELECT * FROM eventi WHERE id = ? AND stato = 'pubblicato'");
...
try {
    ...
    $stmtIns = $conn->prepare(
        "INSERT INTO prenotazioni (evento_id, modalita, referente_nome, referente_cognome, referente_telefono, referente_email, numero_persone, note, token, ip_richiesta)
         VALUES (?,?,?,?,?,?,?,?,?,?)"
    );
    $stmtIns->bind_param(
        "isssssisss",
        $eventoId, $modalita, $referenteNome, $referenteCognome, $referenteTelefono, $referenteEmail, $numeroPersone, $note, $token, $ip
    );
    $stmtIns->execute();
    ...
    if ($tipoPartecipazione === 'propria') {
        // Con tavola propria: solo referente
        $tipoTariffa = 'tavola_propria';
        $importoTariffa = $tariffe['tavola_propria'];

        $stmtPart = $conn->prepare(
            "INSERT INTO prenotazione_partecipanti (prenotazione_id, nome, cognome, email, telefono, tipo_tariffa, importo_tariffa, verificato_db)
             VALUES (?,?,?,?,?,?,?,1)"
        );
        $stmtPart->bind_param(
            "isssssd",
            $prenotazioneId, $referenteNome, $referenteCognome, $referenteEmail, $referenteTelefono, $tipoTariffa, $importoTariffa
        );
        $stmtPart->execute();
        ...
        foreach ($posti as $p) {
            ...
            $tipoInfo = determinaTipoTariffa($conn, $nome, $cognome, $email, $telefono, false);
...
$verificatoDb = 1;
    ...
    $conn->rollback();
    $dettaglioErrore = $e->getMessage() . ' | mysqli_error: ' . $conn->error;
...
$htmlCliente = templateEventoInAttesa($referenteNome, $evento, $tavoleRiepilogo, $linkGestione);
...
$prenotazioneArr = [
    ...
    'totale' => array_sum(array_map(static fn($t) => (float) ($t['importo_tariffa'] ?? 0), $tavoleRiepilogo)),
];
...
<main class="admin-container cliente-submit-main">
    <div class="success-card cliente-submit-card">
        ...
        <p class="cliente-submit-subtitle">La prenotazione è stata salvata. Puoi rimanere su questa pagina e consultare con calma il riepilogo dei prezzi.</p>
